...
const dockerController = {
  ...
  listImages: (req, res) => {
    try {
      const output = execSync('docker image ls --format "{{.Repository}}:{{.Tag}} {{.ID}} {{.Size}} {{.CreatedSince}}"').toString();
  ...
  createContainer: (req, res) => {
    try {
      const { imageName, containerName, ports, volumes, env, command: containerCommand } = req.body;
  ...
  listRunningContainers: (req, res) => {
    try {
      const output = execSync('docker ps --format "{{.ID}} {{.Image}} {{.Names}} {{.Status}} {{.Ports}}"').toString();
  ...
  searchLocalImage: (req, res) => {
    try {
      ...
      if (error.status === 1 || error.message.includes('no matching entries found')) {
        return res.json({
          success: true,
          images: []
        });
