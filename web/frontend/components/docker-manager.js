...
function initializeDockerManager() {
    ...
    const createDockerfileForm = document.getElementById('create-dockerfile-form');
    ...
    const createDockerfileStatus = document.getElementById('create-dockerfile-status');
    ...
    const dockerImagesTable = document.getElementById('docker-images-table').querySelector('tbody');
    ...
    const createContainerStatus = document.getElementById('create-container-status');
    
    // DOM elements - Running Containers
    const refreshContainersBtn = document.getElementById('refresh-containers-btn');
    const dockerContainersTable = document.getElementById('docker-containers-table').querySelector('tbody');
    ...
    const imageSearchResultsHeader = document.getElementById('image-search-results-header');
    const imageSearchResultsTable = document.getElementById('image-search-results-table').querySelector('tbody');
    ...
    if (createDockerfileForm) {
        ...
        if (!savePath || !content) {
            createDockerfileStatus.textContent = 'Please provide both Dockerfile path and content';
            return;
        }
    ...
    if (dockerBuildForm) {
        ...
        if (!path || !name) {
            dockerBuildStatus.textContent = 'Please provide both Dockerfile path and image name';
            return;
        }
    ...
    if (createContainerForm) {
        ...
        const volumesList = volumes ? volumes.split(',').map(v => v.trim()) : [];
                    ...
                    if (data.containers && data.containers.length > 0) {
                        data.containers.forEach(container => {
                            ...
                            tr.innerHTML = `
                                ...
                                <td>
                                    <button class="btn btn-danger btn-sm stop-container" data-id="${container.id}">Stop</button>
                                </td>
                            `;
    ...
    if (searchImageBtn) {
        ...
        .then(data => {
            ...
            if (data.success) {
                    ...
                    data.images.forEach(image => {
                        ...
                        if (searchType === 'local') {
                            ...
                            tr.innerHTML = `
                                ...
                                <td><button class="btn btn-primary btn-sm pull-image" data-name="${image.name}">Pull</button></td>
                            `;
                        }
                    ...
                    if (searchType === 'dockerhub') {
                        document.querySelectorAll('.pull-image').forEach(button => {
                            button.addEventListener('click', function() {
                                ...
                                document.getElementById('pull-image-form').scrollIntoView({ behavior: 'smooth' });
                            });
                        });
                    }
                ...
                imageSearchStatus.textContent = `Error: ${data.message || 'Unknown error'}`;
                console.error('Search error:', data.error);
                
                // Show detailed error in global status
                document.getElementById('global-status').textContent = `Error: ${data.message || 'Unknown error during search'}`;
            }
        })
        .catch(error => {
            imageSearchStatus.textContent = `Error: ${error.message || 'Network error'}`;
            console.error('Error searching for images:', error);
            
            // Show detailed error in global status
            document.getElementById('global-status').textContent = `Error: ${error.message || 'Network error during search'}`;
        })
        .finally(() => {
    ...
    if (pullImageForm) {
        ...
        pullImageStatus.textContent = `Pulling image ${name}${tag ? ':' + tag : ''}...`;
